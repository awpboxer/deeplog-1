...
def generate(name, log_name):
    window_size = 10
    hdfs = {}
    length = 0
    with open(f'../output/{log_name}/' + name, 'r') as f:
        for ln in f.readlines():
            ln = list(map(lambda n: n - 1, map(int, ln.strip().split())))
            ln = ln + [-1] * (window_size + 1 - len(ln)) #padding ln to window size
            hdfs[tuple(ln)] = hdfs.get(tuple(ln), 0) + 1  #count the session
            length += 1
...
class Predicter():
    def __init__(self, model, options):
        ...
        self.num_of_class = options["num_of_class"]
    ...
    def predict_unsupervised(self):
        ...
        test_normal_loader, test_normal_length = generate(f'{self.log_name}_test_normal', self.log_name)
        test_abnormal_loader, test_abnormal_length = generate(
            f'{self.log_name}_test_abnormal', self.log_name)
        TP = 0
        FP = 0
        # Test the model
        start_time = time.time()
        #normal test data. prepocess data to sliding window sequence
        with torch.no_grad():
            for line in tqdm(test_normal_loader.keys()):
                for i in range(len(line) - self.window_size):
                    seq0 = line[i:i + self.window_size]  #seq0: sequentials
                    label = line[i + self.window_size]
                    seq1 = [0] * self.num_of_class   #seq1: quantitatives
                    ...
                    predicted = torch.argsort(output,
                                              1)[0][-self.num_candidates:] #sort output and extract num_candidates
                    if label not in predicted:
                        FP += test_normal_loader[line]
                        break
        #abnomal test data
        with torch.no_grad(): #no_grad:停止自动的梯度计算
    ...
    def predict_supervised(self):
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            output = F.sigmoid(output)[:, 0].cpu().detach().numpy()  # using sigmoid to compute probability
            # predicted = torch.argmax(output, dim=1).cpu().numpy()
            predicted = (output < 0.2).astype(int)  #setting the threshold = 0.2
