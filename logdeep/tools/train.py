...
class Trainer():
    ...
    def train(self, epoch):
        ...
        tbar = tqdm(self.train_loader, desc="\r") #show progress in loop (%) and low overhead
        ...
        for i, (log, label) in enumerate(tbar):
            ...
            output = self.model(features=features, device=self.device) #features[0].shape = (num of sequence, window_size(sequence size), num of dimension)
    ...
    def start_train(self):
        for epoch in range(self.start_epoch, self.max_epoch):
            ...
            self.train(epoch)
            if epoch >= self.max_epoch // 2 and epoch % 2 == 0: #运行完一半EPOCH 且对偶数项epoch进行验证
                self.valid(epoch)
                self.save_checkpoint(epoch,
                                     save_optimizer=True,
